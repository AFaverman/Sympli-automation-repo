 ...
 var bcSfFilterTemplate = {
     ...
     'productGridItemHtml': '<div class="Grid__Cell 1/'+ bcSfFilterConfig.custom.mobile_row + '--phone 1/' + bcSfFilterConfig.custom.tablet_row + '--tablet-and-up 1/' + bcSfFilterConfig.custom.desktop_row + '--' + buildClass() +'">'+
                               '<div data-item-id="{{itemId}}" data-variant-id="{{variantId}}"></div>' +
 ...
 function prepareShopifyData(data) {
   ...
   data.firstVariantId = firstVariant.id;
   return data;
 }
 ...
 BCSfFilter.prototype.buildProductGridItem = function(data, index) {
   ...
   // Add styleMe attributes
   itemHtml = itemHtml.replace(/{{itemId}}/g, data.id);
   itemHtml = itemHtml.replace(/{{variantId}}/g, data.firstVariantId);
 ...
 function buildImage(data, images) {
   try{
     ...
     if (images.length > 0) {
       ...
       if($('body').hasClass('collection-template-page')) {
         var location = window.location.pathname;
         var path = location.split("/");
         var collection = path[path.length-1];
         images.forEach(function(item,index){
           if(typeof item.alt != 'undefined' && item.alt ){
             alt_tag = item.alt.split(" | ");
             alt_tag.forEach(function(alt,i) {
               if(alt == collection) {
                   active_image = item;
               }	
             });
       ...
       var max_width = active_image.width;
       ...
       var aspect_ratio = active_image.width / active_image.height;
       if (use_natural_size) {
       ...
       var support_size = imageSize(sizes_main, active_image);
 
       var thumbUrl_main = images.length > 0 ? bcsffilter.optimizeImage(active_image['src'], '{width}x') : bcSfFilterConfig.general.no_image_url;    
       ...
       htmlImg += '<noscript>';
       htmlImg += '<img class="ProductItem__Image ProductItem__Image--alternate" src="' + bcsffilter.optimizeImage(active_image['src'], '600x') + '" alt="' + data.title + '">';
       htmlImg += '<img class="ProductItem__Image" src="' + bcsffilter.optimizeImage(active_image['src'], '600x') + '" alt="' + data.title + '">';
       htmlImg += '</noscript>';
       htmlImg += '</div>';
       htmlImg += '</a>';
     }
 ...
 function buildPrice(data) {
   try{
     ...
     var main_price = bcsffilter.formatMoney(data.selected_or_first_available_variant.price);
     console.log(data);
     var compare_price = bcsffilter.formatMoney(data.compare_at_price_min);
     ...
     if (onSale) {
       html += '<span class="ProductItem__Price Price Price--highlight Text--subdued" data-money-convertible>$' + Math.round(main_price) + '</span> ';
       html += '<span class="ProductItem__Price Price Price--compareAt Text--subdued" data-money-convertible>$' + Math.round(compare_price) + '</span>';
     } else {
       if (priceVaries) {
         html += '<span class="ProductItem__Price Price Text--subdued">' + bcSfFilterConfig.label.from.replace(/{{min_price}}/g, '$' + Math.round(main_price)) + '</span>';
       } else {
         html += '<span class="ProductItem__Price Price Text--subdued" data-money-convertible>$' + Math.round(main_price) + '</span>';
       }
     }
     html += '</div>';
     return html;
 ...
 function buildInfo(data, indx) {
   try{  
     ...
     if (bcSfFilterConfig.custom.show_product_info) {
       ...
       html += '</h2>';   
       if (bcsffilter.getProductMetafield(data, 'judgeme', 'badge') !== null) {
           html += "<div class='jdgm-widget jdgm-preview-badge' data-id='" + data.id + "'>" + bcsffilter.getProductMetafield(data, 'judgeme', 'badge').replace(/\|/g, '"') + "</div>";
       }
         ...
         if (option_name.indexOf('color') != -1 || option_name.indexOf('colour') != -1 || option_name.indexOf('couleur') != -1) {
           ...
           data.variants.forEach(function (variant) {
             ...
             if (values.indexOf(value) == -1) {
               ...
               if (!imageInfo){
                 ...
                 if($('body').hasClass('collection-template-page')) {
                   ...
                   if(imageInfo.alt != null){
                     if(imageInfo.alt.length > 0 && typeof imageInfo.alt != 'undefined' && imageInfo.alt ){
                       alt_tag = imageInfo.alt.split(" | ");
                       alt_tag.forEach(function(alt,i) {
                         if(alt == collection) {
               ...
               var fileUrlElements = bcSfFilterMainConfig.general.file_url.split("/")
               fileUrlElements[fileUrlElements.length - 1] = name_color +fileUrlElements[fileUrlElements.length - 1]
               var url_color = fileUrlElements.join("/")
 ...
 BCSfFilter.prototype.buildFilterSorting = function() {
   if (bcSfFilterConfig.custom.show_sorting && bcSfFilterTemplate.hasOwnProperty('sortingHtml')) {
 	...
 	delete sortingArr['created-ascending']; 
     if (sortingArr) {
       // Build content
 ...
 BCSfFilter.prototype.buildExtrasProductList = function(data, eventType) {
   ...
   */
   ...
   jQ(".ProductItem__Info .ProductItem__ColorSwatchList .ProductItem__ColorSwatchItem label.ColorSwatch").click(function(){
     ...
     var variantPrice = '';
     var main_price = bcsffilter.formatMoney(variantInfo.data('variant-price'));
     var compare_price = bcsffilter.formatMoney(variantInfo.data('variant-compare-at-price'));
    	main_price = main_price.replace("&#36;", "");
     compare_price = compare_price.replace("&#36;", "");
     if (variantInfo.data('variant-compare-at-price') > variantInfo.data('variant-price')){
       variantPrice += '<span class="ProductItem__Price Price Price--highlight Text--subdued" data-money-convertible="">$' + Math.round(main_price) + '</span>'
       variantPrice += '<span class="ProductItem__Price Price Price--compareAt Text--subdued" data-money-convertible="">$' + Math.round(compare_price) + '</span>';
     } else {
       variantPrice += '<span class="ProductItem__Price Price Text--subdued" data-money-convertible>$' + Math.round(main_price) + '</span>';
     }
 ...
 BCSfFilter.prototype.buildAdditionalElements = function(data, eventType) {
   ...
   if (eventType != 'init') { _usfSetPrimebs();}
